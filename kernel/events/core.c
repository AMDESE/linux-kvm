...
static struct pmu *passthru_core_pmu;
...
static void perf_switch_interrupt(bool enter, u32 guest_lvtpc)
{
	...
	if (!passthru_core_pmu)
		return;

	if (passthru_core_pmu->switch_interrupt &&
	    try_module_get(passthru_core_pmu->module)) {
		passthru_core_pmu->switch_interrupt(enter, guest_lvtpc);
		module_put(passthru_core_pmu->module);
	}
}
...
int perf_pmu_register(struct pmu *pmu, const char *name, int type)
{
	...
	/*
	 * Initialize passthru_core_pmu with the core pmu that has
	 * PERF_PMU_CAP_PASSTHROUGH_VPMU capability.
	 */
	if (!passthru_core_pmu && (pmu->capabilities & PERF_PMU_CAP_PASSTHROUGH_VPMU))
		    passthru_core_pmu = pmu;
