...
static const char *rmp(char *b, u64 pfn)
{
	...
	sprintf(b, " / RMP#%d:%s %s asid%d %s%s%s%s%s",
		segno,
...
#define RMPF(f, n)	(e.f ? n : "")
	...
	if (e.mmio || e.ide_stream_ide)
		sprintf(b + strlen(b), " ide%d", e.ide_stream_ide);
	...
	for (i = 0; i < ARRAY_SIZE(e.vmpl); ++i) {
		if (!e.vmpl[i].r)
			continue;
		sprintf(b + strlen(b), " VMPL%d=%s%s%s%s%s%s%s", i,
...
static int dump_ioptes(u64 pfn, unsigned tabs, const char *pgszs, u64 addr, u64 *ptes)
{
	...
	for (i = 1 /* not zero intentionally */; i <= ARRAY_SIZE(p); ++i) {
		...
		if (p[first].present) {
			if (print_addr(addr + first * lvlpgsz)) {
			    printf(" %s%s\t%d: %llx %d*%s %s%s%s%s%s%s (%x %x)",
				   praddr(ba, addr + first * lvlpgsz),
				   ind,
				   first,
				   pfn1,
...
static int dump_iopdes(u64 pfn, unsigned lvl, unsigned tabs, u64 addr, u64 *ptes)
{
	...
	for (unsigned i = 0; i < ARRAY_SIZE(p); ++i, addr += lvlpgsz) {
		...
		if (g.verbose > 1 || hugepage) {
			printf(">%s%s\t%d: %llx %s%s (%x %x) nxt=%d",
			       praddr(ba, addr),
			       ind,
			       i,
			       pfn1,
...
#define FL(x) ((dte[i].x)?'+':'-')
		printf("%s: h=%llx mode=%d ir%c iw%c gv%c\n",
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "l:d:f:p:q:n:r:R:s:S:e:vchP")) != -1) {
		switch (opt) {
		case 'f':
