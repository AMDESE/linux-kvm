...
def parse_tdi_status(data):
    ...
    valid, state, meas_digest_fresh, meas_digest_valid, all_request_redirect, bind_p2p, lock_msix, no_fw_update = struct.unpack_from('<8B', data, off)
    off += 8
    ...
    off += 8
    # Print as in tsm_tdi_status_user_show
    if not valid:
        print(f"ret=0\nstate={state}:{tdisp_state_to_str(state)}")
        return
